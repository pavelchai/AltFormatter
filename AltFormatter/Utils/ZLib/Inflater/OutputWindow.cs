...
namespace AltFormatter.ZLib
{
    ...
    internal sealed class OutputWindow
    {
        ...
        public void Write(int value)
        {
            ...
            {
                throw new LocalizedException("Window is full.");
            }
        ...
        public void Repeat(int length, int distance)
        {
            ...
            if ((repStart <= border) && (windowEnd < border))
            {
                if (length <= distance)
                {
                    Buffer.BlockCopy(window, repStart, window, windowEnd, length);
                    windowEnd += length;
                }
                else
                {
        ...
        public void CopyDict(byte[] dictionary, int offset, int length)
        {
            if (dictionary == null)
            {
                throw new LocalizedException("Dictionary is null.");
            }

            if (windowFilled > 0)
            {
                throw new LocalizedException("Operation is invalid.");
            }
            ...
            Buffer.BlockCopy(dictionary, offset, window, 0, length);
        ...
        /// </summary>
        /// <returns>NumberLiteral of bytes left in window</returns>
        ...
        /// </summary>
        /// <returns>NumberLiteral of bytes filled</returns>
        ...
        public int CopyOutput(byte[] output, int offset, int len)
        {
            ...
            if (tailLen > 0)
            {
                Buffer.BlockCopy(window, WindowSize - tailLen, output, offset, tailLen);
                offset += tailLen;
                len = copyEnd;
            }
            Buffer.BlockCopy(window, copyEnd - len, output, offset, len);
            windowFilled -= copied;
            if (windowFilled < 0)
            {
                throw new LocalizedException("Operation is invalid.");

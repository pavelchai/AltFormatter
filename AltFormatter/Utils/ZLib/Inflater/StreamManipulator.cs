...
namespace AltFormatter.ZLib
{
    ...
    internal sealed class StreamManipulator
    {
        ...
        public bool TryGetBits(int bitCount, ref int output, int outputOffset)
        {
        ...
        public int CopyBytes(byte[] output, int offset, int length)
        {
            if (length < 0)
            {
                throw new LocalizedException("Length is less than zero.");
            }

            if ((bitsInBuffer_ & 7) != 0)
            {
                // bits_in_buffer may only be 0 or a multiple of 8
                throw new LocalizedException("Bit buffer is not byte aligned!");
            }
            ...
            Buffer.BlockCopy(window_, windowStart_, output, offset, length);
            windowStart_ += length;
        ...
        public void SetInput(byte[] buffer, int offset, int count)
        {
            ...
            if (windowStart_ < windowEnd_)
            {
                throw new LocalizedException("Old input was not completely processed.");
            }
            ...
            {
                throw new LocalizedException("Count is invalid.");
            }
